...
namespace step_driver {
    ...
    void StepDriver::set_speed(this StepDriver& self,
                               ...
                               std::float64_t dt) noexcept
    {
        // auto  error_speed = speed - self.get_speed(dt);
    ...
    void StepDriver::set_acceleration(this StepDriver& self,
                                      std::float64_t acceleration,
                                      std::float64_t dt) noexcept
    {
        auto speed = utility::integrate(acceleration,
    ...
    Microstep StepDriver::speed_to_microstep(std::float64_t speed,
                                             std::float64_t step_change) noexcept
    {
        return Microstep::FULL;
    }

    Direction StepDriver::speed_to_direction(std::float64_t speed) noexcept
    {
        return speed > 0.0F ? Direction::FORWARD : Direction::BACKWARD;
    }

    std::uint32_t StepDriver::speed_to_frequency(std::float64_t speed,
                                                 std::float64_t step_change) noexcept
    {
    ...
    void StepDriver::set_control_speed(this StepDriver& self, std::float64_t control_speed) noexcept
    {
    ...
    void StepDriver::set_microstep(this StepDriver& self, Microstep microstep) noexcept
    {
        self.microstep = microstep;
        self.driver.set_microstep(microstep);
    }

    void StepDriver::set_direction(this StepDriver& self, Direction direction) noexcept
    {
        self.direction = direction;
        self.driver.set_direction(direction);
    }

    void StepDriver::set_frequency(this StepDriver& self, std::uint32_t frequency) noexcept
    {
        self.frequency = frequency;
        self.driver.set_frequency(frequency);
    }

    std::float64_t StepDriver::get_position(this StepDriver& self, std::float64_t dt) noexcept
    {
        return std::fmodf(self.step_change() * static_cast<std::float64_t>(self.step_count),
                          360.0F);
    }

    std::float64_t StepDriver::get_speed(this StepDriver& self, std::float64_t dt) noexcept
    {
        auto position = self.get_position(dt);

        return utility::differentiate(position, std::exchange(self.prev_position, position), dt);
    }

    std::float64_t StepDriver::get_acceleration(this StepDriver& self, std::float64_t dt) noexcept
    {
        auto speed = self.get_speed(dt);

        return utility::differentiate(speed, std::exchange(self.prev_speed, speed), dt);
    }

    bool StepDriver::should_start(this StepDriver& self, std::float64_t control_speed) noexcept
    {
        return (std::abs(control_speed) > MIN_SPEED && std::abs(control_speed) < MAX_SPEED) &&
               self.is_stopped;
    }

    bool StepDriver::should_stop(this StepDriver& self, std::float64_t control_speed) noexcept
    {
        return (std::abs(control_speed) < MIN_SPEED || std::abs(control_speed) > MAX_SPEED) &&
               !self.is_stopped;
    }
