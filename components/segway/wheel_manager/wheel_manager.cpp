...
namespace segway {
    ...
    namespace {
        ...
        WheelDriver& get_wheel_driver(WheelType type) noexcept
        {
        ...
        {
#ifdef USE_QUEUES
            return xQueuePeek(get_queue(QueueType::WHEEL),
                              &event,
                              pdMS_TO_TICKS(1));
#else
        ...
        void set_wheels_speed(std::float64_t left_speed,
                              ...
                              std::float64_t dt) noexcept
        {
            ...
            get_wheel_driver(WheelType::WHEEL_LEFT)
                .set_wheel_speed(left_speed, dt);
            get_wheel_driver(WheelType::WHEEL_RIGHT)
                .set_wheel_speed(right_speed, dt);
        }
        ...
        void process_wheel_event_group_bits() noexcept
        {
            ...
            if ((event_bits & WHEEL_EVENT_BIT_LEFT_STEP_TIMER) ==
                WHEEL_EVENT_BIT_LEFT_STEP_TIMER) {
        ...
        inline void wheel_task_init() noexcept
        {
            ...
            static std::array<StackType_t, WHEEL_TASK_STACK_DEPTH>
                wheel_task_stack = {};
        ...
        inline void wheel_queue_init() noexcept
        {
            ...
            constexpr auto WHEEL_QUEUE_STORAGE_SIZE =
                WHEEL_QUEUE_ITEM_SIZE * WHEEL_QUEUE_ITEMS;

            static StaticQueue_t wheel_queue_buffer = {};
            static std::array<std::uint8_t, WHEEL_QUEUE_STORAGE_SIZE>
                wheel_queue_storage = {};
            ...
            set_queue(QueueType::WHEEL,
                      xQueueCreateStatic(WHEEL_QUEUE_ITEMS,
                                         ...
                                         &wheel_queue_buffer));
#else
            constexpr auto WHEEL_MESSAGE_BUFFER_ITEM_SIZE =
                sizeof(ControlEvent);
        ...
        inline void wheel_periph_init() noexcept
        {
            ...
            auto a4988_gpio_write_pin =
                [](void* user, std::uint16_t pin, bool state) {
                    gpio_write_pin(static_cast<GPIOPin>(pin),
                                   static_cast<GPIOState>(state));
                };
            ...
            auto a4988_pulse_set_freq = [](void* user, std::uint32_t freq) {
                if (freq > 0UL) {
                    ...
                    utility::frequency_to_prescaler_and_period(2 * freq,
                                                               ...
                                                               period);
                    LOG(TAG,
                        "Frequency: %ld, prescaler: %ld, period: %ld",
                        freq,
                        psc,
                        period);
            ...
            a4988::Config a4988_configs[] = {
                [WheelType::WHEEL_LEFT] = {.pin_ms1 = MS1_1,
                                           .pin_ms2 = MS2_1,
                                           .pin_ms3 = MS3_1,
                                           .pin_reset = RESET_1,
                                           .pin_sleep = SLEEP_1,
            ...
            a4988::Interface a4988_interfaces[] = {
                [WheelType::WHEEL_LEFT] = {.gpio_user = nullptr,
                                           ...
                                           .pulse_stop = nullptr,
                                           .pulse_set_freq =
                                               a4988_pulse_set_freq},
                [WheelType::WHEEL_RIGHT] = {
                    .gpio_user = nullptr,
                    .gpio_init = nullptr,
                    .gpio_deinit = nullptr,
            ...
            a4988::A4988 a4988s[] = {
                [WheelType::WHEEL_LEFT] =
                    {.config = a4988_configs[WheelType::WHEEL_LEFT],
                     .interface = a4988_interfaces[WheelType::WHEEL_LEFT]},
                [WheelType::WHEEL_RIGHT] = {
                    .config = a4988_configs[WheelType::WHEEL_RIGHT],
                    .interface = a4988_interfaces[WheelType::WHEEL_RIGHT]}};

            step_driver::StepDriver step_drivers[] = {
                [WheelType::WHEEL_LEFT] = {.driver =
                                               a4988s[WheelType::WHEEL_LEFT],
                                           .steps_per_360 = STEPS_PER_360},
                [WheelType::WHEEL_RIGHT] = {.driver =
