...
namespace segway {
    ...
    inline GPIO_TypeDef* gpio_pin_to_port(GPIOPin pin) noexcept
    {
        ...
        auto index = std::to_underlying(pin) / 16U;
        return (index < GPIO_ADDRESSES.size()) ? GPIO_ADDRESSES[index]
                                               : nullptr;
    }

    inline std::uint16_t gpio_pin_to_mask(GPIOPin pin) noexcept
    {
        return 1U << (std::to_underlying(pin) % 16U);
    }

    GPIOState gpio_read_pin(GPIOPin pin) noexcept
    {
        if (pin != GPIOPin::NC) {
    ...
    void gpio_set_pin(GPIOPin pin) noexcept
    {
        if (pin != GPIOPin::NC) {
            auto port = gpio_pin_to_port(pin);
            auto mask = gpio_pin_to_mask(pin);
    ...
    void gpio_reset_pin(GPIOPin pin) noexcept
    {
        if (pin != GPIOPin::NC) {
            auto port = gpio_pin_to_port(pin);
            auto mask = gpio_pin_to_mask(pin);
    ...
    void gpio_write_pin(GPIOPin pin, GPIOState state) noexcept
    {
        if (pin != GPIOPin::NC) {
            auto port = gpio_pin_to_port(pin);
            auto mask = gpio_pin_to_mask(pin);
    ...
    void gpio_toggle_pin(GPIOPin pin) noexcept
    {
        if (pin != GPIOPin::NC) {
            auto port = gpio_pin_to_port(pin);
            auto mask = gpio_pin_to_mask(pin);
