...
#define __LL_I2C_SPEED_FAST_TO_CCR(__PCLK__, __SPEED__, __DUTYCYCLE__)                                               \
    (uint32_t)(((__DUTYCYCLE__) == LL_I2C_DUTYCYCLE_2)                                                               \
                   ? (((((__PCLK__) / ((__SPEED__) * 3U)) & I2C_CCR_CCR) == 0U) ? 1U                                 \
 ...
 * Max_Data=0x0F (Digital filter enabled and filtering capability up to 15*TPCLK1) This parameter is used to configure
 * the digital noise filter on SDA and SCL input. The digital filter will suppress the spikes with a length of up to
 * DNF[3:0]*TPCLK1.
 * @retval None
 */
...
LL_I2C_ConfigSpeed(I2C_TypeDef* I2Cx, uint32_t PeriphClock, uint32_t ClockSpeed, uint32_t DutyCycle)
{
