...
#define SPDIFRX_CHANNELSTATUS_OFF                                          \
    ((uint32_t)0x00000000U) /* The channel status and user bits are copied \
                               into the SPDIF_DR */
...
HAL_StatusTypeDef
HAL_SPDIFRX_ReceiveDataFlow(SPDIFRX_HandleTypeDef* hspdif, uint32_t* pData, uint16_t Size, uint32_t Timeout);
HAL_StatusTypeDef
HAL_SPDIFRX_ReceiveControlFlow(SPDIFRX_HandleTypeDef* hspdif, uint32_t* pData, uint16_t Size, uint32_t Timeout);
...
#define IS_SPDIFRX_MAX_RETRIES(RET)                                                                               \
    (((RET) == SPDIFRX_MAXRETRIES_NONE) || ((RET) == SPDIFRX_MAXRETRIES_3) || ((RET) == SPDIFRX_MAXRETRIES_15) || \
...
#define IS_PREAMBLE_TYPE_MASK(VAL) (((VAL) == SPDIFRX_PREAMBLETYPEMASK_ON) || ((VAL) == SPDIFRX_PREAMBLETYPEMASK_OFF))

#define IS_VALIDITY_MASK(VAL) (((VAL) == SPDIFRX_VALIDITYMASK_OFF) || ((VAL) == SPDIFRX_VALIDITYMASK_ON))

#define IS_PARITY_ERROR_MASK(VAL) (((VAL) == SPDIFRX_PARITYERRORMASK_OFF) || ((VAL) == SPDIFRX_PARITYERRORMASK_ON))

#define IS_SPDIFRX_CHANNEL(CHANNEL) (((CHANNEL) == SPDIFRX_CHANNEL_A) || ((CHANNEL) == SPDIFRX_CHANNEL_B))
...
#define IS_STEREO_MODE(MODE) (((MODE) == SPDIFRX_STEREOMODE_DISABLE) || ((MODE) == SPDIFRX_STEREOMODE_ENABLE))

#define IS_CHANNEL_STATUS_MASK(VAL) (((VAL) == SPDIFRX_CHANNELSTATUS_ON) || ((VAL) == SPDIFRX_CHANNELSTATUS_OFF))
