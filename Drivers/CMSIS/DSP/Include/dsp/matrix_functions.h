 ...
 */
arm_status
arm_mat_scale_q15(const arm_matrix_instance_q15* pSrc, q15_t scaleFract, int32_t shift, arm_matrix_instance_q15* pDst);
 ...
 */
arm_status
arm_mat_scale_q31(const arm_matrix_instance_q31* pSrc, q31_t scaleFract, int32_t shift, arm_matrix_instance_q31* pDst);
 ...
 * @param[out] dst   points to the instance of the output floating-point matrix structure.
 * @return The function returns ARM_MATH_SIZE_MISMATCH, if the dimensions do not match.
 * If the input matrix does not have a decomposition, then the algorithm terminates and returns error status
 * ARM_MATH_DECOMPOSITION_FAILURE. If the matrix is ill conditioned or only semi-definite, then it is better using the
 * LDL^t decomposition. The decomposition is returning a lower triangular matrix.
 */
 ...
 * @return The function returns ARM_MATH_SIZE_MISMATCH, if the dimensions do not match.
 * If the input matrix does not have a decomposition, then the algorithm terminates and returns error status
 * ARM_MATH_DECOMPOSITION_FAILURE. The decomposition is returning a lower triangular matrix.
 */
arm_status arm_mat_ldlt_f32(const arm_matrix_instance_f32* src,
 ...
 * ARM_MATH_DECOMPOSITION_FAILURE. The decomposition is returning a lower triangular matrix.
 */
arm_status arm_mat_ldlt_f64(const arm_matrix_instance_f64* src,
