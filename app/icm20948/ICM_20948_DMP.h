...
#define DATA_OUT_CTL2 \
    (4 * 16 + 2) // 16-bit: Data output control 2 register : configure the BM, accel/gyro/compass accuracy and gesture
                 // such as Pick-up
#define DATA_INTR_CTL \
    (4 * 16 +         \
     12) // 16-bit: Determines which sensors can generate interrupt according to bit map defined for DATA_OUT_CTL1
#define FIFO_WATERMARK \
    (31 * 16 + 14) // 16-bit: DMP will send FIFO interrupt if FIFO count > FIFO watermark. FIFO watermark is set to 80%
                   // of actual FIFO size by default
...
// To do this and output hardware unit again as configured FSR, write 0x4000000 to ACC_SCALE DMP register, and write
// 0x40000 to ACC_SCALE2 DMP register.
...
#define INV_NEEDS_ACCEL_MASK                                                                                           \
    ((1L << 1) | (1L << 3) | (1L << 9) | (1L << 10) | (1L << 11) | (1L << 15) | (1L << 17) | (1L << 18) | (1L << 19) | \
     (1L << 20) | (1L << 23) | (1L << 25) | (1L << 29) | (1L << 30) | (1L << 31))
#define INV_NEEDS_GYRO_MASK                                                                                            \
    ((1L << 3) | (1L << 4) | (1L << 9) | (1L << 10) | (1L << 11) | (1L << 15) | (1L << 16) | (1L << 25) | (1L << 26) | \
     (1L << 29) | (1L << 30) | (1L << 31))
#define INV_NEEDS_COMPASS_MASK \
    ((1L << 2) | (1L << 3) | (1L << 11) | (1L << 14) | (1L << 20) | (1L << 24) | (1L << 25) | (1L << 31))
...
#define INV_NEEDS_ACCEL_MASK1 \
    ((1L << 3) | (1L << 5) | (1L << 6) | (1L << 7) | (1L << 9) | (1L << 10)) // I.e. 35, 37, 38, 39, 41, 42
...
#define icm_20948_DMP_Gyro_Calibr_Bytes \
    12 // lcm20948MPUFifoControl.c suggests icm_20948_DMP_Gyro_Calibr_Bytes is not supported?
...
#define icm_20948_DMP_Fsync_Detection_Bytes \
    2 // lcm20948MPUFifoControl.c suggests icm_20948_DMP_Fsync_Detection_Bytes is not supported?
...
typedef struct {
    ...
    uint8_t ALS[icm_20948_DMP_ALS_Bytes]; // Byte[0]: Dummy, Byte[2:1]: Ch0DATA, Byte[4:3]: Ch1DATA, Byte[6:5]: PDATA,
                                          // Byte[7]: Dummy
    ...
    // 9-axis quaternion data and Geomag rv is always followed by 2-bytes of heading accuracy, hence the size of Quat9
    // and Geomag data size in the FIFO is 14 bytes. Quaternion data for both cases is cumulative/integrated values. For
    // a given quaternion Q, the ordering of its elements is {Q1, Q2, Q3}. Each element is represented using Big Endian
    // byte order. Q0 value is computed from this equation: Q20 + Q21 + Q22 + Q23 = 1. In case of drift, the sum will
    // not add to 1, therefore, quaternion data need to be corrected with right bias values. The quaternion data is
    // scaled by 2^30.
    union {
