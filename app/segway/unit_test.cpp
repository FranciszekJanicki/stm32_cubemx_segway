...
namespace Segway {
    ...
    void test_imu() noexcept
    {
        ...
        while (1) {
            if (gpio_pin6_exti) {
                ...
                auto const& [r, p, y] = rpy.value_or(Utility::Vector3D{0.0F32, 0.0F32, 0.0F32});
                std::printf("roll: %f, pitch: %f, yaw: %f\n\r", (float)r, (float)p, (float)y);
    ...
    void test_both_steppers() noexcept
    {
        ...
        while (1) {
            if (tim2_period_elapsed) {
                driver_1.set_speed(i += 1.0F, SAMPLING_TIME);
                driver_2.set_speed(i -= 1.0F, SAMPLING_TIME);
                tim2_period_elapsed = false;
                HAL_TIM_Base_Start_IT(&htim2);
            }

            if (tim1_pulse_finished) {
                driver_1.update_step_count();
                tim1_pulse_finished = false;
                HAL_TIM_PWM_Start_IT(&htim1, TIM_CHANNEL_1);
            }

            if (tim3_pulse_finished) {
                driver_2.update_step_count();
                tim3_pulse_finished = false;
                HAL_TIM_PWM_Start_IT(&htim3, TIM_CHANNEL_1);
            }
        }
