...
namespace Segway {
    ...
    {
        self.run_segway(dot_tilt, tilt, dot_rotation, rotation, position, whells_speed, dt);
    }
                            ...
                            std::float32_t const dt) noexcept
    {
        ...
        auto const meas_dot_tilt = Utility::differentiate(meas_tilt, prev_tilt, dt);
        auto const meas_rotation = self.get_rotation();
        auto const meas_dot_rotation = Utility::differentiate(meas_rotation, prev_rotation, dt);
        auto const meas_position = self.get_position(dt);
        auto const meas_whells_speed = Utility::differentiate(meas_position, prev_position, dt);
        ...
        int_e_dot_tilt += Utility::integrate(e_dot_tilt, prev_e_dot_tilt, dt);
        int_e_tilt += Utility::integrate(e_tilt, prev_e_tilt, dt);
        int_e_dot_rotation += Utility::integrate(e_dot_tilt, prev_e_dot_tilt, dt);
        int_e_rotation += Utility::integrate(e_dot_tilt, prev_e_dot_tilt, dt);
        int_e_position += Utility::integrate(e_dot_tilt, prev_e_dot_tilt, dt);
        int_e_whells_speed += Utility::integrate(e_dot_tilt, prev_e_dot_tilt, dt);
        ...
        auto const right_wheel_speed =
            (Ki[0] * int_e_dot_tilt - Kx[1][0] * meas_dot_tilt) + (Ki[1] * int_e_tilt - Kx[1][1] * meas_tilt) +
            (Ki[2] * int_e_dot_rotation - Kx[1][2] * meas_dot_rotation) +
            (Ki[3] * int_e_rotation - Kx[1][3] * meas_rotation) + (Ki[4] * int_e_position - Kx[1][4] * meas_position) +
            (Ki[5] * int_e_whells_speed - Kx[1][5] * meas_whells_speed);

        self.set_wheel_speed(WheelType::LEFT, left_wheel_speed, dt);
        self.set_wheel_speed(WheelType::RIGHT, right_wheel_speed, dt);
    }
